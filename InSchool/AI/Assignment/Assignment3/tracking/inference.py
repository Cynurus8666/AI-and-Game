...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance == None:
            ...
            # Check every legal position
            for pos in self.legalPositions:
                # Calculate the distance from legal position to pacman position
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        """
        "*** YOUR CODE HERE ***"
        # Init a probility board
        allPossible = util.Counter()
        # Check every legal position for calculating the probility
        for oldPos in self.legalPositions:
            # Calculate the distance
            newPosDist = self.getPositionDistribution(
                self.setGhostPosition(gameState, oldPos))
            for newPos, prob in newPosDist.items():
                # Calculate the new probility
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # Count temp for stopping
        count = 0
        # Init an array to save position
        self.particles = []
        # Loop from 0 to numParticles
        while count < self.numParticles:
            # Check each position
            for pos in self.legalPositions:
                if count < self.numParticles:
                    # if position valid, add it to particles
    ...
    def observe(self, observation, gameState):
        ...
        # Is the position is observation, pass it
        if noisyDistance == None:
            self.particles = [self.getJailPosition()]*self.numParticles
        else:
            # init the probility board
            allPossible = util.Counter()
            # get the before probility value
            oldBelief = self.getBeliefDistribution()
            # Browser all position in the legal position
            for pos in self.legalPositions:
                # Calculate the distance from this position to the pacman position
                distance = util.manhattanDistance(pos, pacmanPosition)
                # Calculate the new probility
                allPossible[pos] += emissionModel[distance]*oldBelief[pos]
            # If no position to calculate the probility, init the default
            if not any(allPossible.values()):
                self.initializeUniformly(gameState)
            else:
                # Calculate the partical probility
                temp = []
                for count in range(0, self.numParticles):
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        posPosition = list(itertools.product(
            self.legalPositions, repeat=self.numGhosts))
        random.shuffle(posPosition)

        count = 0
        self.particles = []
        while count < self.numParticles:
            for pos in posPosition:
                if count < self.numParticles:
                    self.particles.append(pos)
                    count += 1
                else:
